...
const sourceAccount = '0x9999999999999999999999999999999999999999' as `0x${string}`
...
function source(overrides: Partial<Awaited<ReturnType<ReceiptSource['getTransaction']>>> & { receiptStatus?: 'success' | 'reverted'; chainId?: number } = {}): ReceiptSource {
  return {
    ...
    getTransaction: async () => ({ from: sourceAccount, to: overrides.to ?? recipient, value: overrides.value ?? 1000000000000000n, chainId: overrides.chainId ?? 84532 }),
  }
}
...
describe('verifyNativeTransfer', () => {
  it('accepts a matching successful native transfer', async () => {
    ...
    expect(result).toMatchObject({ verified: true, blockNumber: '123', sourceAddress: sourceAccount })
  })
